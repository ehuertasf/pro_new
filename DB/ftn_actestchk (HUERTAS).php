...
function ftn_act_estadochk($codsol,$loguser){
	...
	while($jfila_=mysql_fetch_array($jquery_)){
		...
		$codper=$jfila_[1];
		mysql_query("delete from tmp_estperchk where codper=$codper and codsol=$codsol and loguser='".$loguser."';") or die(mysql_error());
		mysql_query("delete from tmp_estactchk where codper=$codper and codsol=$codsol and loguser='".$loguser."';") or die(mysql_error());
		...
		while($ifila_=mysql_fetch_array($iquery_)){
			...
			while($ofila_=mysql_fetch_array($oquery_)){
				mysql_query("insert into tmp_estperchk (codsol,codper,codchk,codestchk,fe_mov,loguser) values ($codsol,$codper,$codchk,'".$ofila_[0]."',NOW(),'".$loguser."')") or die(mysql_error());
			}
		...
		while($wfila_=mysql_fetch_array($wquery_)){
			if($wfila_[1]!=3){
				mysql_query("insert into tmp_estactchk (codsol,codper,codestchk,fe_mov,loguser) values ($codsol,$codper,$wfila_[1],NOW(),'".$loguser."')") or die(mysql_error());
				break;
			}else{
				mysql_query("insert into tmp_estactchk (codsol,codper,codestchk,fe_mov,loguser) values ($codsol,$codper,3,NOW(),'".$loguser."')") or die(mysql_error());

                                mysql_query("update tb_detallesolicitud set codestchk=3 where codsol=$codsol and codper=$codper and codpacchk='".$jfila_[2]."'") or die(mysql_error());
